...
app.post('/api/bookings', async (req, res) => {
  try {
    ...
    try {
      ...
      // Use enhanced email generator for multi-tour support
      let emailHtml;
      try {
        console.log("Attempting to use enhanced email generator for multi-tour support");
        const fs = require('fs');
        const path = require('path');
        const enhancedGeneratorPath = path.join(__dirname, 'enhanced-email-generator.js');
        
        if (fs.existsSync(enhancedGeneratorPath)) {
          const enhancedGenerator = require('./enhanced-email-generator.js');
          emailHtml = enhancedGenerator.generateEnhancedBookingEmail(result.rows[0], tourDetails);
          console.log("✅ Using enhanced email generator with multi-tour support");
        } else {
          emailHtml = generateBookingEmail(result.rows[0], tourDetails);
          console.log("⚠️ Enhanced generator not found, using original");
        }
      } catch (error) {
        console.error("❌ Error with enhanced email generator:", error);
        emailHtml = generateBookingEmail(result.rows[0], tourDetails);
      }
